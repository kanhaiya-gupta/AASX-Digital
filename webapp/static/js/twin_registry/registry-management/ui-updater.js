...
export default class TwinRegistryUIUpdater {
    ...
    updateStatistics(stats) {
        ...
        if (activeTwinsElement) {
            activeTwinsElement.textContent = stats.status_distribution?.active || 0;
        }
        ...
        if (activeAlertsElement) {
            activeAlertsElement.textContent = stats.health_distribution?.critical || 0;
        }
        ...
        if (analyticsHealthyTwins) {
            analyticsHealthyTwins.textContent = stats.health_distribution?.healthy || 0;
        }
        if (analyticsWarningTwins) {
            analyticsWarningTwins.textContent = stats.health_distribution?.warning || 0;
        }
        if (analyticsCriticalTwins) {
            analyticsCriticalTwins.textContent = stats.health_distribution?.critical || 0;
        }
    ...
    createTwinTableRow(twin) {
        ...
        row.innerHTML = `
            ...
            <td>
                <div class="fw-bold">${twin.twin_name}</div>
