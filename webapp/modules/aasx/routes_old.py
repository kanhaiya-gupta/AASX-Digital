...
async def get_aasx_stats():
    ...
    try:
        projects = project_manager.list_projects()
        total_files = sum(len(project_manager.list_project_files(p.get('id', ''))) for p in projects)
        
        return {
            "total_projects": len(projects),
            "total_files": total_files,
            "processed_files": total_files,  # Simplified for now
            "success_rate": 100.0,
    ...
    try:
        ...
        for project in projects:
            ...
            for file_info in files:
                ...
                file_info['project_name'] = project.get('name', 'Unknown')
...
async def list_use_cases():
    ...
    try:
        ...
        use_cases = project_manager.list_use_cases()
        ...
        print(f"Error getting use cases from database: {e}")
        # No fallback - database is required
        raise HTTPException(
            status_code=500, 
            detail=f"Database error: Failed to retrieve use cases. Please ensure the database is properly initialized and run the physics use cases update script."
        )
