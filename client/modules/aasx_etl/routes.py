...
async def aasx_dashboard(request: Request):
    """AASX ETL Dashboard - PUBLIC ACCESS for development and testing"""
    
    # Try to get user context if available (for authenticated users)
    ...
    default_context = {
        ...
        "is_demo": False,
        "has_permission": lambda perm: False,  # Public users have no permissions
        "get_user_type": lambda: "public"
    }
    
    # Use user context if available, otherwise use default
    current_user = user_context if user_context else default_context
    
    try:
        # Try to get services if user is authenticated
        if user_context:
            ...
            # Public access - show empty/default data
            user_projects = []
            user_files = []
            user_stats = {
                "total_projects": 0,
        ...
        logger.warning(f"Could not get user data: {e}, using default values")
    ...
    return request.app.state.templates.TemplateResponse(
        "aasx/index.html",
        {
            ...
            "user": current_user,
            "can_upload": current_user.get("has_permission", lambda x: False)("write"),
            "can_process": current_user.get("has_permission", lambda x: False)("write"),
            "can_delete": current_user.get("has_permission", lambda x: False)("manage") and not current_user.get("is_demo", False),
            "user_type": current_user.get("get_user_type", lambda: "public")(),
            "user_projects": user_projects,
            "user_files": user_files,
            "user_stats": user_stats,
            "organization_stats": organization_stats,
            "is_demo": current_user.get("is_demo", False),
            "is_public": user_context is None
        }
    )
