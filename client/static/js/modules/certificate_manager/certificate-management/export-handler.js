...
export class ExportHandler {
    ...
    async waitForAuthSystem() {
        console.log('🔐 Export Handler: Waiting for central auth system...');
        
        if (window.authSystemReady && window.authManager) {
            console.log('🔐 Export Handler: Auth system already ready');
            return;
        }
        
        return new Promise((resolve) => {
            const handleReady = () => {
                console.log('🚀 Export Handler: Auth system ready event received');
            ...
            setTimeout(() => {
                clearInterval(checkInterval);
                window.removeEventListener('authSystemReady', handleReady);
                console.warn('⚠️ Export Handler: Timeout waiting for auth system');
                resolve();
            }, 10000);
        });
    }
    ...
    updateAuthState() {
        if (window.authManager) {
            ...
            console.log('🔐 Export Handler: Auth state updated', {
            ...
            this.currentUser = null;
            this.authToken = null;
            console.log('🔐 Export Handler: No auth manager available');
        }
    }
    ...
    clearSensitiveData() {
        ...
        this.exportQueue = [];
        this.isExporting = false;
        console.log('🧹 Export Handler: Sensitive data cleared');
    }
