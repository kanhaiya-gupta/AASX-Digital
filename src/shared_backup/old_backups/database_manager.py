...
class DatabaseProjectManager:
        ...
        try:
            self.cursor.execute("""
                ...
                WHERE type='table' AND name IN (
                    ...
                    'twin_events', 'twin_health', 'sync_history', 'use_cases', 'project_use_cases'
                )
            """)
    ...
    def get_file_info(self, file_id: str, project_id: str = None) -> Optional[Dict[str, Any]]:
        """
        Get file information by ID from database.
        
        Args:
            file_id: Unique file identifier
            project_id: Optional project ID for additional validation (not required since file_id is unique)
        """
        try:
            if project_id:
                # If project_id provided, use it for additional validation
                self.cursor.execute("""
                    SELECT * FROM project_files 
                    WHERE file_id = ? AND project_id = ?
                """, (file_id, project_id))
            else:
                # Since file_id is unique, we can find it without project_id
                self.cursor.execute("""
                    SELECT * FROM project_files 
                    WHERE file_id = ?
                """, (file_id,))
    ...
    def upload_file(self, project_id: str, file_path: Path, original_filename: str, 
        ...
        # Check if file already exists in project (by original filename)
        existing_file = self.find_file_by_name(project_id, original_filename)
        if existing_file:
            # File already exists - update it instead of creating duplicate
            print(f"⚠️ File '{original_filename}' already exists in project. Updating existing file.")
            
            # Update the existing file
            return self._update_existing_file(
    ...
    def _update_existing_file(self, project_id: str, existing_file_id: str, 
                             new_file_path: Path, description: Optional[str] = None, 
                             uploaded_by: str = None) -> Dict[str, Any]:
        """Update an existing file with new content."""
        try:
            # Get existing file info
            existing_file = self.get_file_info(existing_file_id, project_id)
            if not existing_file:
                raise DatabaseManagerError(f"Existing file {existing_file_id} not found")
            
            # Copy new file to replace existing
            project_dir = self.get_project_dir(project_id)
            destination_path = project_dir / existing_file['filename']
            
            try:
                shutil.copy2(new_file_path, destination_path)
            except Exception as e:
                raise DatabaseManagerError(f"Failed to copy new file: {e}")
            ...
            updates = {
                "size": file_size,
                "updated_at": datetime.now().isoformat(),
                "status": "not_processed"  # Reset status for reprocessing
            }
            ...
            self.cursor.execute("""
                UPDATE project_files 
                SET size = ?, updated_at = ?, status = ?, description = ?
                WHERE file_id = ? AND project_id = ?
            """, (
                updates["size"],
                updates["updated_at"],
                updates["status"],
                updates.get("description", existing_file.get("description", "")),
                existing_file_id,
                project_id
            ))
            ...
            updated_file = self.get_file_info(existing_file_id, project_id)
            updated_file["id"] = existing_file_id  # Ensure ID is included
            
            return updated_file
            
        except Exception as e:
            raise DatabaseManagerError(f"Failed to update existing file: {e}")
    ...
    def register_digital_twin(self, project_id: str, file_id: str, twin_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            ...
            file_id: File identifier (will also be used as twin_id)
        ...
        """
        try:
            # Get file info using the updated method that works with just file_id
            file_info = self.get_file_info(file_id, project_id)
            if not file_info:
                return {'success': False, 'error': 'File not found'}
            
            # Check if twin already exists for this file_id
            existing_twin = self.get_twin_by_id(file_id)
            if existing_twin:
                # Check if the existing twin is active
                if existing_twin.get('status') == 'active':
                    return {'success': False, 'error': 'Active twin already exists for this file'}
                else:
                    # Twin exists but is not active (e.g., orphaned), so we can register it
                    print(f"Found existing twin with status '{existing_twin.get('status')}', proceeding with registration")
            
            # Use file_id as twin_id for perfect 1:1 relationship
            twin_id = file_id
            
            # Get project name for twin display name
            project_metadata = self.get_project_metadata(project_id)
            project_name = project_metadata.get('name', 'Unknown_Project')
            
            # Get use case information for better twin naming
            use_case_name = "Unknown_Use_Case"
            try:
                # Get use case through the link table
                self.cursor.execute("""
                    SELECT uc.name FROM use_cases uc
                    JOIN project_use_cases puc ON uc.use_case_id = puc.use_case_id
                    WHERE puc.project_id = ?
                """, (project_id,))
                
                use_case_row = self.cursor.fetchone()
                if use_case_row:
                    use_case_name = use_case_row[0]
            except Exception:
                # If we can't get use case, use default
                pass
            
            # Prepare twin data
            twin_record = {
                'twin_id': twin_id,  # Same as file_id
                'aasx_filename': file_info['filename'],
                'project_id': project_id,
                'aas_id': twin_data.get('aas_id'),
                'twin_name': twin_data.get('twin_name', f"{use_case_name} - {project_name} - {file_info['filename']}"),
            ...
            return {
                'success': True,
                'twin_id': twin_id,  # Same as file_id
                'file_id': file_id,  # Explicitly show the relationship
    ...
    def get_twin_by_id(self, twin_id: str) -> Optional[Dict[str, Any]]:
        """Get twin information by twin_id (which is the same as file_id)."""
        try:
            self.cursor.execute("""
            ...
            if row:
                ...
                if twin_info.get('metadata'):
                    try:
                        twin_info['metadata'] = json.loads(twin_info['metadata'])
            ...
            raise DatabaseManagerError(f"Failed to get twin by ID: {e}")
    ...
    def update_digital_twin(self, project_id: str, file_id: str, twin_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            project_id: Project identifier
            file_id: File identifier (same as twin_id)
            twin_data: Updated digital twin data
        """
        try:
            # Get file info using the updated method
            file_info = self.get_file_info(file_id, project_id)
            if not file_info:
                return {'success': False, 'error': 'File not found'}
            
            # Find existing twin using file_id as twin_id
            existing_twin = self.get_twin_by_id(file_id)
            if not existing_twin:
                return {'success': False, 'error': 'Twin not found for this file'}
            
            twin_id = file_id  # Same as file_id
            
            # Update twin data
            self.cursor.execute("""
                UPDATE twin_relationships 
                SET aas_id = ?, twin_name = ?, twin_type = ?, 
                    metadata = ?, data_points = ?, last_sync = CURRENT_TIMESTAMP
                WHERE twin_id = ?
            """, (
    ...
    def create_use_case(self, use_case_data: Dict[str, Any]) -> str:
        ...
        Args:
            use_case_data: Dictionary containing use case information
                - name: Use case name
                - description: Use case description
                - category: Category (thermal, structural, fluid_dynamics, multi_physics, industrial)
                - industry: Industry name
                - famous_examples: JSON string of famous examples
                - optimization_targets: JSON string of optimization targets
                - materials: JSON string of materials
                - complexity: Complexity level
                - expected_duration: Expected processing duration
        ...
        Returns:
            use_case_id: The created use case ID
        """
        try:
            use_case_id = use_case_data.get('use_case_id') or str(uuid.uuid4())
            
            self.cursor.execute("""
                INSERT INTO use_cases (
                    use_case_id, name, description, category, industry, 
                    famous_examples, optimization_targets, materials, complexity,
                    expected_duration, data_points, physics_type, tags, metadata
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                use_case_id,
                use_case_data.get('name'),
                use_case_data.get('description'),
                use_case_data.get('category'),
                use_case_data.get('industry'),
                json.dumps(use_case_data.get('famous_examples', [])) if isinstance(use_case_data.get('famous_examples'), list) else use_case_data.get('famous_examples'),
                json.dumps(use_case_data.get('optimization_targets', [])) if isinstance(use_case_data.get('optimization_targets'), list) else use_case_data.get('optimization_targets'),
                json.dumps(use_case_data.get('materials', [])) if isinstance(use_case_data.get('materials'), list) else use_case_data.get('materials'),
                use_case_data.get('complexity'),
                use_case_data.get('expected_duration'),
                use_case_data.get('data_points'),
                use_case_data.get('physics_type'),
                json.dumps(use_case_data.get('tags', [])) if isinstance(use_case_data.get('tags'), list) else use_case_data.get('tags'),
                json.dumps(use_case_data.get('metadata', {})) if isinstance(use_case_data.get('metadata'), dict) else use_case_data.get('metadata')
            ))
            ...
            self.conn.rollback()
            raise DatabaseManagerError(f"Failed to create use case: {e}")
    
    def get_use_case(self, use_case_id: str) -> Optional[Dict[str, Any]]:
        """Get a use case by ID"""
        try:
            self.cursor.execute("SELECT * FROM use_cases WHERE use_case_id = ?", (use_case_id,))
            row = self.cursor.fetchone()
            
            if row:
                use_case = dict(row)
                # Parse JSON fields
                for field in ['famous_examples', 'optimization_targets', 'materials', 'tags', 'metadata']:
                    if use_case[field]:
                        try:
                            use_case[field] = json.loads(use_case[field])
                        except json.JSONDecodeError:
                            pass
                return use_case
            return None
            ...
            raise DatabaseManagerError(f"Failed to get use case: {e}")
    
    def list_use_cases(self, category: str = None) -> List[Dict[str, Any]]:
        """List all use cases, optionally filtered by category"""
        try:
            if category:
                self.cursor.execute("SELECT * FROM use_cases WHERE category = ? ORDER BY name", (category,))
            else:
                self.cursor.execute("SELECT * FROM use_cases ORDER BY category, name")
            ...
            raise DatabaseManagerError(f"Failed to list use cases: {e}")
    ...
    def update_use_case(self, use_case_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for key, value in updates.items():
                if key in ['famous_examples', 'optimization_targets', 'materials', 'tags', 'metadata']:
            ...
            values.append(use_case_id)  # WHERE clause
            ...
            self.conn.rollback()
            raise DatabaseManagerError(f"Failed to update use case: {e}")
    
    def delete_use_case(self, use_case_id: str) -> bool:
        """Delete a use case"""
        try:
            self.cursor.execute("DELETE FROM use_cases WHERE use_case_id = ?", (use_case_id,))
            self.conn.commit()
            return self.cursor.rowcount > 0
            
        except Exception as e:
            self.conn.rollback()
            raise DatabaseManagerError(f"Failed to delete use case: {e}")
    
    def link_project_to_use_case(self, project_id: str, use_case_id: str) -> bool:
        """Link a project to a use case"""
        try:
            self.cursor.execute("""
                INSERT OR IGNORE INTO project_use_cases (project_id, use_case_id)
                VALUES (?, ?)
            """, (project_id, use_case_id))
            self.conn.commit()
            return True
            
        except Exception as e:
            self.conn.rollback()
            raise DatabaseManagerError(f"Failed to link project to use case: {e}")
    
    def unlink_project_from_use_case(self, project_id: str, use_case_id: str) -> bool:
        """Unlink a project from a use case"""
        try:
            self.cursor.execute("""
                DELETE FROM project_use_cases 
            ...
            self.conn.rollback()
            raise DatabaseManagerError(f"Failed to unlink project from use case: {e}")
    ...
    def get_project_use_cases(self, project_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise DatabaseManagerError(f"Failed to get project use cases: {e}")
    
    def get_use_case_projects(self, use_case_id: str) -> List[Dict[str, Any]]:
        """Get all projects linked to a use case"""
        try:
            self.cursor.execute("""
                SELECT p.* FROM projects p
                JOIN project_use_cases puc ON p.project_id = puc.project_id
            ...
            raise DatabaseManagerError(f"Failed to get use case projects: {e}")
    
    def get_use_case_statistics(self) -> Dict[str, Any]:
        """Get statistics about use cases"""
        try:
            # Total use cases
            self.cursor.execute("SELECT COUNT(*) FROM use_cases")
            total_use_cases = self.cursor.fetchone()[0]
            ...
            # Total project-use case links
            self.cursor.execute("SELECT COUNT(*) FROM project_use_cases")
            total_links = self.cursor.fetchone()[0]
            
            # Most popular use cases
            self.cursor.execute("""
                SELECT uc.name, uc.category, COUNT(puc.project_id) as project_count
                FROM use_cases uc
                LEFT JOIN project_use_cases puc ON uc.use_case_id = puc.use_case_id
                GROUP BY uc.use_case_id
                ORDER BY project_count DESC
                LIMIT 10
            """)
            popular_use_cases = [dict(row) for row in self.cursor.fetchall()]
            
            return {
                'total_use_cases': total_use_cases,
                'categories': categories,
                'total_links': total_links,
                'popular_use_cases': popular_use_cases
            }
            
        except Exception as e:
            raise DatabaseManagerError(f"Failed to get use case statistics: {e}") 

    def get_file_trace_info(self, file_id: str) -> Optional[Dict[str, Any]]:
        """
        Get complete trace information from file_id back to use case and project names.
        Since file_id = twin_id, this also includes twin information.
        
        Returns:
            Dictionary containing:
            - file_info: Complete file information
            - project_info: Project name and metadata
            - use_case_info: Use case name and metadata
            - twin_info: Digital twin information (if exists)
            - trace_path: Complete hierarchy path
        """
        try:
            # First, get file info by file_id only
            self.cursor.execute("""
                SELECT * FROM project_files 
                WHERE file_id = ?
            """, (file_id,))
            ...
            self.cursor.execute("""
                SELECT project_id, name, description, metadata FROM projects 
                WHERE project_id = ?
            """, (project_id,))
            ...
            # Get use case information through the link table
            self.cursor.execute("""
                SELECT uc.* FROM use_cases uc
                JOIN project_use_cases puc ON uc.use_case_id = puc.use_case_id
                WHERE puc.project_id = ?
            ...
            use_case_info = None
            if use_case_row:
                use_case_info = dict(use_case_row)
                # Parse JSON fields
                for field in ['famous_examples', 'optimization_targets', 'materials', 'tags', 'metadata']:
                    if use_case_info.get(field):
                        try:
                            use_case_info[field] = json.loads(use_case_info[field])
                        except (json.JSONDecodeError, TypeError):
                            pass
            
            # Get twin information (since file_id = twin_id)
            twin_info = self.get_twin_by_id(file_id)
            ...
            trace_path = {
                ...
                "use_case_id": use_case_info.get('use_case_id') if use_case_info else None,
                "use_case_name": use_case_info.get('name') if use_case_info else None,
                "use_case_category": use_case_info.get('category') if use_case_info else None,
                "twin_id": file_id,  # Same as file_id
                "twin_name": twin_info.get('twin_name') if twin_info else None,
                "twin_status": twin_info.get('status') if twin_info else None
            }
            ...
            raise DatabaseManagerError(f"Failed to get file trace info: {e}")

    def get_file_info_by_id(self, file_id: str) -> Optional[Dict[str, Any]]:
        """Get file information by file_id only (without requiring project_id)."""
        try:
            self.cursor.execute("""
                SELECT * FROM project_files 
                WHERE file_id = ?
            ...
            raise DatabaseManagerError(f"Failed to get file info by ID: {e}")
