    ...
    async def get_by_id(self, metric_id: int) -> Optional[AIRagMetrics]:
        ...
        try:
            async with self.connection_manager.get_session() as session:
                ...
                result = await session.execute(query, {"metric_id": metric_id})
                row = result.fetchone()
                
                if row:
                    return AIRagMetrics.from_dict(dict(row))
                return None
    ...
    async def get_by_registry_id(self, registry_id: str) -> List[AIRagMetrics]:
        ...
        try:
            async with self.connection_manager.get_session() as session:
                ...
                return [AIRagMetrics.from_dict(dict(row)) for row in rows]
    ...
    async def delete(self, metric_id: int) -> bool:
        ...
        try:
            async with self.connection_manager.get_session() as session:
                ...
                await session.execute(query, {"metric_id": metric_id})
                await session.commit()
